...
namespace D3D11Framework
{
	...
	void InputMgr::Close()
	{
		if(!m_Listener.empty()) m_Listener.clear();
		Log::Get()->Debug("Input close");
	}
	...
	void InputMgr::Run(const UINT &msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_KEYDOWN:
			...
			ToUnicode(wParam, HIWORD(lParam) & 0xFF, lpKeyState, buffer, 1, 0); //см. ниже
	...
	void InputMgr::m_eventcursor()
	{
		...
		if(m_curx == Position.x && m_cury == Position.y) return;
		...
		{
			if(!(*it)) continue;
			else if((*it)->MouseMove(MouseEvent(m_curx, m_cury)) == true) return;
		}
	}
	...
	void InputMgr::m_eventmouse(const eMouseKeyCodes Code, bool press)
	{
		for(auto it = m_Listener.begin(); it != m_Listener.end(); ++it)
		{
			if(!(*it)) continue;
			if(press == true){
				if((*it)->MousePressed(MouseEventClick(Code, m_curx, m_cury)) == true) return;
			} else{
				if((*it)->MouseReleased(MouseEventClick(Code, m_curx, m_cury)) == true) return;
			}
		}
	}
	...
	void InputMgr::m_mousewheel(short Value)
	{
		...
		{
			if(!(*it)) continue;
			else if((*it)->MouseWheel(MouseEventWheel(m_MouseWheel, m_curx, m_cury)) == true) return;
		}
	}
	...
	void InputMgr::m_eventkey(const eKeyCodes KeyCode, const wchar_t ch, bool press)
	{
		for(auto it = m_Listener.begin(); it != m_Listener.end(); ++it)
		{
			if(!(*it)) continue;
			if(press == true){
				if((*it)->KeyPressed(KeyEvent(ch, KeyCode)) == true) return;
			} else{
				if((*it)->KeyReleased(KeyEvent(ch, KeyCode)) == true) return;
			}
		}
	}
}