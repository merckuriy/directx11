...
void Frustum::ConstructFrustum(float screenDepth, CXMMATRIX projectionMatrix, CXMMATRIX viewMatrix)
{
	...
	//Вычисляем мин дистанцию по Z в фрустуме.
	//???
	...
	// Создание матрицы фрустума из видовой и обновленной проекционной матриц.
...
{
	for(int i = 0; i < 6; i++){
		//Скалярное произведение плоскости на вектор.
...
{
	for(int i = 0; i < 6; i++){
		float ret = XMVectorGetX(XMPlaneDotCoord(m_planes[i], 
			XMVectorSet(xCenter-size, yCenter-size, zCenter-size, 1.0f)));
		if(ret >= 0.0f) continue;

		ret = XMVectorGetX(XMPlaneDotCoord(m_planes[i],
			XMVectorSet(xCenter+size, yCenter-size, zCenter-size, 1.0f)));
		if(ret >= 0.0f) continue;
...
bool Frustum::CheckSphere(float xCenter, float yCenter, float zCenter, float radius)
{
	for(int i = 0; i < 6; i++){
		float ret = XMVectorGetX(XMPlaneDotCoord(m_planes[i], 
			XMVectorSet(xCenter, yCenter, zCenter, 1.0f)));
	...
	float xSize, float ySize, float zSize)
{
	for(int i = 0; i < 6; i++){
		float ret = XMVectorGetX(XMPlaneDotCoord(m_planes[i],
			XMVectorSet(xCenter - xSize, yCenter - ySize, zCenter - zSize, 1.0f)));
		if(ret >= 0.0f) continue;
